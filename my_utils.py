...
def make_graph(data):
	...
	Output: 
		...
		graph_inv_sb: graph_inv sorted from smallest to largest.
		optimal: int.
...
def quicksort(edgewts):
	quicksort_drive(edgewts, 0, len(edgewts) - 1)

def quicksort_drive(somewts, start, end):
	if start >= end:
		return
	p = partition(somewts, start, end)
	quicksort_drive(somewts, start, p - 1)
	quicksort_drive(somewts, p + 1, end)

def partition(somewts, start, end):
	b = a = start
	while a < end:
...
def initialize_tsp_lp(graph, graph_inv, optimal, n, m):
	...
	tsp_lp.setParam('OutputFlag', False)
